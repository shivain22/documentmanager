...
/**
 * Performance test for the DocColValueStore entity.
 */
class DocColValueStoreGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DocColValueStore entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all docColValueStores")
            .get("/api/doc-col-value-stores")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new docColValueStore")
            .post("/api/doc-col-value-stores")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "colValue":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_docColValueStore_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created docColValueStore")
                .get("${new_docColValueStore_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created docColValueStore")
            .delete("${new_docColValueStore_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
