...
/**
 * Performance test for the DocColNameStore entity.
 */
class DocColNameStoreGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DocColNameStore entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all docColNameStores")
            .get("/api/doc-col-name-stores")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new docColNameStore")
            .post("/api/doc-col-name-stores")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "colName":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_docColNameStore_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created docColNameStore")
                .get("${new_docColNameStore_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created docColNameStore")
            .delete("${new_docColNameStore_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
