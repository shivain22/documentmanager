...
/**
 * Performance test for the DocStore entity.
 */
class DocStoreGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DocStore entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all docStores")
            .get("/api/doc-stores")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new docStore")
            .post("/api/doc-stores")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created docStore")
                .get("${new_docStore_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created docStore")
            .delete("${new_docStore_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
