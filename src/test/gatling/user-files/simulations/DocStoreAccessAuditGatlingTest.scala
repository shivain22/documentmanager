...
/**
 * Performance test for the DocStoreAccessAudit entity.
 */
class DocStoreAccessAuditGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DocStoreAccessAudit entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all docStoreAccessAudits")
            .get("/api/doc-store-access-audits")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new docStoreAccessAudit")
            .post("/api/doc-store-access-audits")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_docStoreAccessAudit_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created docStoreAccessAudit")
                .get("${new_docStoreAccessAudit_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created docStoreAccessAudit")
            .delete("${new_docStoreAccessAudit_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
